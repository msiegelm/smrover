...
from .misc import Bunch,num2dt64

def reshape_var(ds,nsecb=3600,ti = []):
    ns =  np.timedelta64( ds.time[1].values-ds.time[0].values ,'ms').astype(int)/1000
    fs = int(1/ns)
    nt = len(ds.time.values)
    lb = nsecb*fs
    nb = int(np.floor(nt/lb))
    ntt = int(lb*nb)
    dresh = np.reshape(ds.dep.values[:ntt],(lb,nb),order="F")
    presh = np.reshape(ds.pdbar.values[:ntt],(lb,nb),order="F")

    treshnum = np.reshape(dates.date2num(ds.time.values[:ntt]),(lb,nb),order="F")
    tresh = np.reshape(ds.time.values[:ntt],(lb,nb),order="F")

    tmn = np.nanmean(treshnum,axis=0)
    dmn = np.nanmean(dresh,axis=0)
    pmn = np.nanmean(presh,axis=0)

    out = Bunch()
    out.tnum_mn = tmn
    out.t64_mn = num2dt64(out.tnum_mn)
    out.dep_mn = dmn
    out.depp_mn = out.dep_mn-np.nanmean(out.dep_mn)
    out.pdbar_mn = pmn
    out.dep = dresh
    out.t64 = tresh
    out.tnum = treshnum
    ...
    if np.any(ti):
        out.dep = np.interp(ti,out.tnum_mn,out.dep_mn)
        out.depp = np.interp(ti,out.tnum_mn,out.depp_mn)
        out.pdbar = np.interp(ti,out.tnum_mn,out.pdbar_mn)
        out.tnum_mn = ti
        out.t64_mn = num2dt64(out.tnum)
    return out
...
def _convolve(a, b, axis=-1):
    return np.apply_along_axis(np.convolve, axis, a, b, mode='valid')

def wave_energy_density(dep,dt,t64=[],nperseg=None,noverlap=0,nsmooth=7,nfft=None):
    """
    ...
    dt : float
        Time step (s)
    t64 : array
        Time array (datetime64, same shape as dep). (optional: if you want t64 to be included in output for 2d Edens)
    nperseg : int or None
    ...
    if dep.ndim == 1:
        ...
        if nperseg == None:
            nperseg = len(dep[:,0])
        t_det = signal.detrend(dep,axis=0)
        ff, Spp = signal.welch(t_det, fs=fs, nperseg=nperseg, noverlap=noverlap,return_onesided=True,nfft=nfft,axis=0)
        ff = ff[1:]
        Spp = Spp[1:,:]
        if nsmooth is not None:
            window = np.ones((nsmooth),dtype=float)/float(nsmooth)
            Spp = _convolve(Spp,window,axis=0)
            ff = _convolve(ff,window,axis=0)
