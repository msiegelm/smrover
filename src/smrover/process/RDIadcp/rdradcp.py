...
References: https://stompy.readthedocs.io/en/latest/_modules/stompy/io/rdradcp.html
"""
...
# from adcp_ptools import datetime2matlabdn
...
def rd_hdr(fd, msg=msg_print):
    ...
    while len(cfgid) < 2 or (cfgid[0] != 0x7F or cfgid[1] != 0x7F) or not checkheader(fd):
        ...
        if len(nextbyte) == 0:  # End of file
            msg('RD_HDR: EOF reached before finding valid cfgid\n')
            hdr = None
            return hdr, pos
        ...
        if pos % 10000 == 0:
            msg("RD_HDR: Still looking for valid cfgid at file position %d...\n" % pos)
        # end
    # end
...
def rd_fixseg(fd):
    ...
    # print(f"progver is {cfg.prog_ver}")
    # 8,9,16 - WH navigator
    # 10 -rio grande
    # 15, 17 - NB
    ...
    if int(cfg.prog_ver) in (4, 5):
        cfg.name = 'bb-adcp'
    elif int(cfg.prog_ver) in (8, 9, 10, 16, 50, 51, 52, 77):
        cfg.name = 'wh-adcp'
    ...
    elif int(cfg.prog_ver) == 77: # support for WHII
        cfg.cpu_serialnum = fromfile(fd, uint8, 8) # cpu board serial
        cfg.sysbandwidth = fromfile(fd,uint8,2)
        cfg.syspower = fromfile(fd, uint8, 1)[0]
        fd.seek(1,os.SEEK_CUR) # spare
        cfg.inst_serialnum = fromfile(fd, uint8, 4)
        cfg.beam_angle = fromfile(fd, uint8, 1)[0]
        nbyte += 17
...
def rd_buffer(fd, num_av, msg=msg_print, verbose=True):
    ...
    # so.
    from smrover.process.RDIadcp.process import datetime2matlabdn
    ...
    while k + 1 < num_av:
        ...
        byte_offset = nbyte + 2
        datatypes=f"# data types is {len(hdr.dat_offsets)}"
        blocklen=f"Blocklen is {hdr.nbyte}"
        # print(datatypes)
        # print(blocklen)
        ...
        for n in range(len(hdr.dat_offsets)):  # n: 1 => 0 based
            ...
            if id_ == '0000':
                ...
                if cfg.name == 'bb-adcp':
                    ...
                    if int(cfg.prog_ver) in (8, 10, 16, 50, 51, 52, 77):
            ...
            # end # id_==0600 # bottom track
            ...
            elif id_ == '5901':  # Binary ISM information (not implemented)
                fd.seek(19, os.SEEK_CUR)
                nbyte = 2 + 19
                ...
                mtime = datetime2matlabdn(datetime.datetime(utim[2] + utim[3] * 256, utim[1], utim[0]))
                ...
                fd.seek(6, os.SEEK_CUR)
                utim = fromfile(fd, uint8, 4)
                mtime = datetime2matlabdn(datetime.datetime(utim[0] + utim[1] * 256, utim[3], utim[2]))
...
def nmedian(x, window=inf, dim=None):
    ...
    if n1 == 1:  # each column has only one row - no stats to be taken, just copy.
        ...
        if n2 == 1:
            ...
            kk[ll] = 0;
            x[:, ll] = nan  # in those cases, the answer is nan.  seems harsh.
